...
if not os.path.exists("vanshika_screenshots(7)"):
    os.makedirs("vanshika_screenshots(7)")
...
try:
    while True:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if distance < 0.6:
                ...
                screenshot_filename = f"vanshika_screenshots(7)/{name}_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpeg"
                if attentive == 'Yes':
        ...
        if (datetime.now() - last_save_time).total_seconds() >= 30:
            df.to_excel('vanshika_attendance_with_attention_score(7).xlsx', index=False)
            print("DataFrame saved to 'vanshika_attendance_with_attention_score(7).xlsx'")
    ...
    if not df.empty:
        df.to_excel('vanshika_attendance_with_attention_score(7).xlsx', index=False)
        print("Final attendance with attentiveness saved to 'vanshika_attendance_with_attention_score(7).xlsx'.")
