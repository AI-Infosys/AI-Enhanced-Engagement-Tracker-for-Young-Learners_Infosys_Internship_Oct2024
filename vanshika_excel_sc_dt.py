...
if not os.path.exists("vanshika_screenshots(5)"):
    os.makedirs("vanshika_screenshots(5)")
...
try:
    while True:
        ...
        for face_encoding in face_encodings:
            ...
            if distance < confidence_threshold:
                ...
                if last_recognition_time is None or (now - last_recognition_time) >= entry_duration:
                    ...
                    screenshot_filename = f"vanshika_screenshots(5)/Vanshika{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
        ...
        if (datetime.now() - last_saved_time).total_seconds() >= 30:
            df.to_excel('vanshika_attendance_with_screenshots_dt(5).xlsx', index=False)
            print("Data saved to 'vanshika_attendance_with_screenshots_dt(5).xlsx'.")
    ...
    if not df.empty:
        df.to_excel('vanshika_attendance_with_screenshots_dt(5).xlsx', index=False)
        print("Attendance and screenshots saved to 'vanshika_attendance_with_screenshots_dt(5).xlsx'.")
