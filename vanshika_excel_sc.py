...
if not os.path.exists("vanshika_screenshots(4)"):
    os.makedirs("vanshika_screenshots(4)")
...
try:
    while True:
        ...
        for face_encoding in face_encodings:
            ...
            if distance < confidence_threshold:
                ...
                if last_recognition_time is None or (now - last_recognition_time) >= entry_duration:
                    # Save screenshot
                    screenshot_filename = f"vanshika_screenshots(4)/Vanshika{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                    cv.imwrite(screenshot_filename, frame)  # Save the screenshot

                    # Log a new entry for Vanshika with screenshot
                ...
                # Draw a rectangle around the face and label it as Vanshika
                for face_location in face_locations:
                    top, right, bottom, left = face_location
                    cv.rectangle(frame, (left, top), (right, bottom), (0, 255, 0), 2)
                    cv.putText(frame, 'Vanshika', (left, top - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5,
                               (255, 0, 0), 2, cv.LINE_AA)

        if not recognized:
            # If the face is not recognized, label it as 'Not Vanshika'
            cv.putText(frame, 'Not Vanshika', (30, 55), cv.FONT_HERSHEY_SIMPLEX, 1,
                       (255, 0, 0), 2, cv.LINE_AA)
        ...
        if (datetime.now() - last_save_time).total_seconds() >= 30:
            df.to_excel('vanshika_attendance_with_screenshots(4).xlsx', index=False)
            print("DataFrame saved to 'vanshika_attendance_with_screenshots(4).xlsx'")
    ...
    if not df.empty:
        df.to_excel('vanshika_attendance_with_screenshots(4).xlsx', index=False)
        print("Final attendance and screenshots saved to 'vanshika_attendance_with_screenshots(4).xlsx'.")
